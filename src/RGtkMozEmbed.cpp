...
SEXP R_InitGTKMozEmbed()
{
  ...
  //http://old.nabble.com/nsIWebBrowser-and-GtkMozEmbed-td26678850.html
...
SEXP R_NewMozGtkBrowser()
{
  ...
  SEXP ans, klass;
  /*
  PROTECT(klass = MAKE_CLASS("nsIWebBrowser"));
  PROTECT(ans = NEW(klass));
  SET_SLOT(ans, Rf_install("ref"), 
	   R_MakeExternalPtr(webBrowser, 
			     Rf_install("nsIWebBrowser"), 
			     R_NilValue));
  */
  PROTECT(klass = MAKE_CLASS("MozGtkBrowser"));
  PROTECT(ans = NEW(klass));
  SET_SLOT(ans, Rf_install("ref"),
	   R_MakeExternalPtr(browser, 
			     Rf_install("MozEmbedBrowser"), 
			     R_NilValue));
  UNPROTECT(2);
  return(ans);
...
extern "C"
SEXP R_gtk_moz_embed_get_nsIWebBrowser(SEXP ibrowser)
{
  
  TestGtkBrowser *browser = (TestGtkBrowser *) 
    R_ExternalPtrAddr(GET_SLOT(ibrowser, Rf_install("ref")));
  
  nsIWebBrowser *webBrowser;
  gtk_moz_embed_get_nsIWebBrowser(GTK_MOZ_EMBED(browser->mozEmbed), &webBrowser);
  
  //NS_IF_ADDREF(webBrowser); function addrefs it already.
  SEXP ans, klass, ptr;
  
  PROTECT( klass = MAKE_CLASS( "nsIWebBrowser" ) );
  PROTECT( ans = NEW( klass ) );
  PROTECT( ptr = R_MakeExternalPtr( webBrowser, 
			     Rf_install( "nsIWebBrowser" ), 
			     R_NilValue ) );
...
extern "C"
SEXP R_LoadURL(SEXP ibrowser, SEXP URL)
{
  char const *url = CHAR(STRING_ELT(URL,0));
