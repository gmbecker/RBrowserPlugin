...
#include <stdio.h>
//Directory structure changed between 1.9.1 and 1.9.2 so the following is 
//not suported moving forward.
//#include "xpcom/xptcall.h" We replace it with our compatability file
...
/* Added by DTL. Not certain what the best way to deal with the copyright,
   e.g. create a separate file and put the class defn in a separate header or just add this. 
*/
#include "Rinternals.h"
extern "C"
SEXP
R_new_InvokeTestTarget()
{
   InvokeTestTarget *test = new InvokeTestTarget();
   return(R_MakeExternalPtr((void *) test, Rf_install("InvokeTestTarget"), R_NilValue));
}
